...
public class ApplicationContext implements Closeable{
	...
	public Object getBean(String name){
		...
		return getBean(bean.getPrototype(),bean);
	}
	
	public Object getBean(Class<?> prototype){
		return getBean(prototype,ContextObject.getBeanInfo(prototype.getName()));
	}
	
	private Object getBean(Class<?> prototype,BeanInfo bean){
		...
		if (instance == null) {
			//如果构造函数未注册则创造一个实例
			try {
				instance = BeanContextFactory.getInstance().getBean(prototype);
			} catch (Exception e) {
				log.error(StackTraceInfo.getTraceInfo() + e.getMessage());
				throw new ApplicationException(e);
			}
...
//			//如果构造函数未注册则创造一个实例
//			try {
//				instance = BeanContextFactory.getInstance().getBean(bean.getPrototype());
//			} catch (Exception e) {
//				log.error(StackTraceInfo.getTraceInfo() + e.getMessage());
//				throw new ApplicationException(e);
//			}
